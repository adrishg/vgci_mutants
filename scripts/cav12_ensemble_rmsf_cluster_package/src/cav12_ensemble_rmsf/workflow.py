...
def inspect_inputs(cfg: Mapping[str, Any], output_dir: str | Path, max_models_per_dataset: int = 3) -> dict[str, Path]:
    ...
    manifest_path = output_dir / "cav12_model_manifest.csv"
    manifest.to_csv(manifest_path, index=False)
    counts_path = output_dir / "cav12_dataset_counts.csv"
    if manifest.empty:
    ...
    dataset_status_path = output_dir / "cav12_dataset_path_status.csv"
    ...
    fasta_status_path = output_dir / "cav12_fasta_status.csv"
    ...
    structure_inventory_path = output_dir / "cav12_structure_inventory.csv"
    ...
    reference_inventory_path = output_dir / "cav12_reference_inventory.csv"
...
def prepare_references(cfg: Mapping[str, Any], output_dir: str | Path) -> dict[str, Path]:
    ...
    bundle_path = output_dir / "cav12_aligned_references.npz"
    np.savez_compressed(
        bundle_path,
    ...
    report_path = output_dir / "cav12_reference_alignment_report.csv"
    ...
    for axis_index, raw in enumerate(raw_numbers.tolist()):
        ...
        row = {
            ...
            "mutation_site": any(name.endswith("_mutation_site") for name in region_names),
            "mutation_centered_window": any("window" in name.lower() for name in region_names),
            "mask_annotation_status": "deferred",
        }
        for condition, fasta in fasta_by_condition.items():
            row[f"residue_identity_{condition}"] = fasta[axis_index] if axis_index < len(fasta) else ""
        annotation_rows.append(row)
    annotations_path = output_dir / "cav12_residue_annotations.csv"
...
def merge_shards(
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    paths = {
        "coordinates": output_dir / "cav12_aligned_ca_coordinates.npy",
        "present": output_dir / "cav12_aligned_ca_present.npy",
        "identities": output_dir / "cav12_aligned_residue_identities.npy",
        "metadata": output_dir / "cav12_alignment_metadata.csv",
        "raw_numbers": output_dir / "cav12_raw_residue_numbers.npy",
    }
    ...
    failed_path = output_dir / "cav12_failed_models.csv"
    metadata.loc[metadata["alignment_success"] != True].to_csv(failed_path, index=False)  # noqa: E712
    summary_path = output_dir / "cav12_merge_summary.json"
...
def calculate_profiles(
    ...
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    metadata = pd.read_csv(merged_dir / "cav12_alignment_metadata.csv")
    ...
    coords = np.load(merged_dir / "cav12_aligned_ca_coordinates.npy", mmap_mode="r")
    present = np.load(merged_dir / "cav12_aligned_ca_present.npy", mmap_mode="r")
    identities = np.load(merged_dir / "cav12_aligned_residue_identities.npy", mmap_mode="r")
    raw_numbers = np.load(merged_dir / "cav12_raw_residue_numbers.npy")
    ...
    selected_manifest_path = output_dir / f"cav12_{subset_name}_selected_models.csv"
    ...
    profiles_path = output_dir / f"cav12_{subset_name}_per_residue_profiles.csv"
    ...
    comparisons_path = output_dir / f"cav12_{subset_name}_protocol_vs_vanilla.csv"
    ...
    for group_keys, group in profiles.groupby(["dataset", "sequence_condition", "protocol"], sort=True):
        ...
        region_masks = {
            ...
            "all_mutation_sites": group["mutation_site"].fillna(False).to_numpy(dtype=bool),
            "all_mutation_centered_windows": group["mutation_centered_window"].fillna(False).to_numpy(dtype=bool),
        }
        annotation_text = group["annotation_regions"].fillna("").astype(str)
        for block in cfg.get("annotations", {}).get("raw_regions", []):
            region_name = str(block["name"])
            region_masks[region_name] = annotation_text.str.split(";").apply(lambda names: region_name in names).to_numpy(dtype=bool)
    ...
    summary_path = output_dir / f"cav12_{subset_name}_group_summary.csv"
    ...
    per_model_path = output_dir / f"cav12_{subset_name}_per_model_reference_rmsd.csv"
    selected[[
